...
from .layers import IBN, SwitchNorm2d
...
class DWConvLayer(nn.Sequential):
    def __init__(self, in_channels, out_channels, stride=1, bias=False, sn=False):
        super().__init__()
        out_ch = out_channels
        groups = in_channels
        kernel = 3
...
class ConvLayer(nn.Sequential):
    def __init__(self, in_channels, out_channels, kernel=3, stride=1, dropout=0.1, bias=False, ibn=False, sn=False):
        super().__init__()
        out_ch = out_channels
        groups = 1
        ...
        if ibn:
          self.add_module('ibn', IBN(out_ch))
        elif sn:
          self.add_module('sn', SwitchNorm2d(out_ch))
        else:
...
class KingBlock(nn.Module):
    ...
    def get_link(self):  # calculate the linkcount of all layers
    ...
    def __init__(self, in_channels, n_layers, dwconv=False):
        ...
        self.concate_out = {3:[2],4:[2],6:[2,4],8:[2,6],9:[3,6],10:[2,5,8],11:[3,5,9],12:[3,6,9],15:[3,6,9,12],16:[2,6,10,14],18:[2,6,10,16]}
...
class KingNet(nn.Module):
    def __init__(self, arch=53, depth_wise=False, pretrained=False):
        ...
        # stem
        self.base.append(ConvLayer(in_channels=4, out_channels=first_ch[0], kernel=3, stride=2, bias=False, ibn=True))
        self.base.append(ConvLayer(in_channels=first_ch[0], out_channels=first_ch[1], kernel=3, ibn=False, sn=True))
        ...
        for i in range(blks):
            ...
            blk = KingBlock(ch, n_layers[i], dwconv=depth_wise) # ibn doesn't work well in these blocks
            ouch = blk.get_out_ch()
        ...
        if(pretrained):
            ...
            weight_file = 'weights/bc_exp_1,1_val_iou_0.7551_epoch_0.pth'
...
# %%
def kingnet53(arch=53, pretrained=False, **kwargs):
    if arch==53:
        print("KingNet53 LOADED")
        # kingnet53 was trained on ImageNet-1K
        model = KingNet(arch=53, pretrained=False)
    elif arch == 69:
